...
export const PostProvider = ({ children }) => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      console.log('Fetched posts:', data.map(post => ({
        _id: post._id,
  ...
  const addComment = async (postId, text) => {
    try {
      const { data } = await axios.post(`${backend_url}/api/posts/${postId}/comments`, { text });
  ...
  const updateComment = async (postId, commentId, text) => {
    try {
      const { data } = await axios.put(`${backend_url}/api/posts/${postId}/comments/${commentId}`, { text });
      setPosts(posts.map((p) => (p._id === postId ? data : p)));
  ...
  const deleteComment = async (postId, commentId) => {
    try {
      const { data } = await axios.delete(`${backend_url}/api/posts/${postId}/comments/${commentId}`);
      setPosts(posts.map((p) => (p._id === postId ? data : p)));
  ...
  const updatePost = async (postId, text) => {
    try {
      const { data } = await axios.put(`${backend_url}/api/posts/${postId}`, { text });
      setPosts(posts.map((p) => (p._id === postId ? data : p)));
